...
> 补充：
> 一个问题是，函数在严格模式下似乎是块级作用域的。在非严格模式下，如果一个函数被定义在一个块内，在这个块被运行到之前，其标识符的值被初始化为`undefined`（类似`var`)；进入块后，函数被构建，然后函数作用域内可用。它甚至可以与一个块前面的`let`变量同名。
```
'use strict'
{
...
> 注意这里的表述的函数在VariableEnvironment中像是非严格模式的情况。也有说法（或许更新）函数在LexicalEnvironment中
